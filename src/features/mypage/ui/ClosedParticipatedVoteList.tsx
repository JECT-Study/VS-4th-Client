...
const sortingOptions: Array<{ value: ClosedVoteSortType; label: string }> = [
  { value: "LATEST", label: "최신순" },
  { value: "END_AT", label: "오래된 순" },
];

export function ClosedParticipatedVoteList() {
  const [sortingOption, setSortingOption] = useState<ClosedVoteSortType>(sortingOptions[0]!.value);
  const { data, isLoading } = useQuery(closedParticipatedVotesQueryOptions(sortingOption));
